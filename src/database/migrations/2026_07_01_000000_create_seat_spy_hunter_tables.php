...
class CreateSeatSpyHunterTables extends Migration
{
    public function up()
    {
        if (Schema::hasTable('seat_intel_settings') && !Schema::hasTable('seat_spy_hunter_settings')) {
            return;
        }

        if (Schema::hasTable('seat_spy_hunter_settings')) {
            return;
        }
        ...
        Schema::create('seat_spy_hunter_entities', function (Blueprint $table) {
            ...
            $table->unique(['entity_id', 'category']);
        ...
        Schema::create('seat_spy_hunter_character_reports', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('account_user_id')->unique();
            ...
            $table->index(['score', 'rating']);
            $table->index(['corporation_id', 'alliance_id']);
            $table->index('character_id');
            $table->index('user_id');
        });
        ...
        Schema::create('seat_spy_hunter_character_report_evidence', function (Blueprint $table) {
            $table->id();
            $table->foreignId('report_id')->constrained('seat_spy_hunter_character_reports')->cascadeOnDelete();
