...
class EvidenceScoreGuide
{
    ...
    public function rows(): array
    {
        ...
        return [
            ...
            $this->row('shared_ip', 'Shared IP', (string) $this->settings->sharedIpScore(), 'Public login IPs shared with other SeAT user accounts.', sprintf('Configured weight, currently %d. Private, reserved, localhost, and Docker/internal IPs are skipped.', $this->settings->sharedIpScore())),
            $this->row('vpn_ip', 'VPN / Proxy', (string) $this->settings->vpnScore(), 'Public login IPs marked by VPNAPI.io or manual intelligence as VPN, proxy, Tor, or hosting.', sprintf('Configured weight, currently %d. VPNAPI.io results are cached indefinitely by IP.', $this->settings->vpnScore())),
            ...
            $this->row('thin_footprint_cap', 'Thin Footprint Cap', '0 / mitigation', 'Prevents quiet-account style evidence from dominating the score by itself.', 'If thin-footprint signals exceed 35 points together, the excess is subtracted as mitigation. Hostile, IP, employment, killmail, and contract evidence are not affected by this cap.'),
            $this->row('late_token_after_join', 'Late Token After Join', '8', 'A linked character token was added after the account was already represented in a monitored group.', 'Small context signal. This may be normal account maintenance, but it helps reviewers spot alts that were added after joining.'),
            $this->row('post_join_hostile_activity', 'Post-Join Hostile Activity', '12', 'Hostile mail, wallet, market, or contract evidence dated after the account joined a monitored group.', 'Timing context signal. Post-join hostile activity is more relevant than old pre-join history.'),
            ...
            $this->row('score_explanation', 'Score Explanation', '0', 'Shows final score, mitigations, suppressed evidence count, and top score drivers.', 'Context only. Adds no suspicion points.'),
