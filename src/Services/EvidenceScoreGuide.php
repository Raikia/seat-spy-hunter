...
class EvidenceScoreGuide
{
    ...
    public function rows(): array
    {
        ...
        return [
            $this->row('hostile_employment_overlap', 'Hostile Employment Overlap', '2-100', 'Employment history overlap between account characters and current members of configured hostile groups.', '100 if both left the same corp within 10 days. 45 for same-time overlap in the last 2 years. 25 when both were in the same corp during the last 2 years but not at the same time. Otherwise capped at 10.'),
            ...
            $this->row('prejoin_killmail_cluster', 'Pre-Join Killmail Clustering', '16-45', 'Repeated pre-join killmail activity with the same hostile or non-monitored same-side group.', 'Only fires when the same outside group appears across at least 2 days or at least 3 killmails before the character/account joined. Hostile clusters score 28 + 3 per killmail + 2 per active day, capped at 45. Non-monitored outside clusters score 16 + 2 per killmail + 1 per active day, capped at 30.'),
